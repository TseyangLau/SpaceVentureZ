...
class Display:
    ...
    def __init__(self):
        ...
        #pygame.mixer.pre_init(20000, -16, 1, 512)  # option 2 - decent sound quality
    ...
    def menu(self):
        ...
        screen = pygame.image.load('game_images/space_back.jpg').convert()
        ...
        while start:
            ...
            '''get mouseover state'''
            start_mouseover = start_game_b.x + start_game_b.w > (x, y)[0] > start_game_b.x and start_game_b.y + start_game_b.h > (x, y)[1] > start_game_b.y
        ...
        while self.pause:
            ...
            for event in pygame.event.get():
                ...
                if event.type == MOUSEBUTTONDOWN:
                    if event.button == 1:
                        self.user_click = True
                        if res_rect.collidepoint(x, y): # reset button handling
                            self.pause = False
                            self.user_click = False
                            pygame.mixer.music.set_volume(0.5)  # increase music volume
                        if re_rect.collidepoint(x,y): # resume button handling
                            self.pause = False
                            self.user_click = False
                            self.restart()
